...
<template>
  <div class="container mx-auto px-4 my-16 lg:w-3/4 overflow-hidden">
    <h3 class=" relative text-white text-right font-fahkwang text-3xl w-1/2 whitespace-nowrap tracking-widest">
      Projects
    </h3>
    ...
    <div class="flex my-4 gap-2 items-end">
      ...
      <button tupe="button"
        ...
        :class="{ 'bg-gradient-to-t from-teal-500  to-blue-700': showProjects == 'All' }" @click="changeFilter('All')">
    ...
    <div class="w-full pb-4 flex flex-wrap">
      <div v-for="project in projectsArray" :key="project.id" class="flex flex-wrap justify-start items-start p-4 mt-12 w-full
          border border-white rounded-lg shadow text-white
          hover:shadow-lg hover:shadow-cyan-500/50" :class="{ hidden: hideProject(Array.from(project.categories)) }">
        ...
        <div class="w-full p-2 xl:w-1/2 xl:pr-2">
          ...
          <div class="projectLangs flex my-2 gap-1 flex-wrap">
            <div class="relative group" v-for="lang in project.ProgrammingLanguages">
              <img class="h-10 inline rounded-md p-1 hover:bg-white/25" :src="getImageUrl(`${lang}.svg`)" />
              <div
                class="absolute hidden group-hover:block top-1/2 left-1/2 z-50 bg-white text-slate-950 rounded-lg px-2 text-center font-semibold">
                {{ lang }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="w-full">
          ...
          <div class="projectLinks">
            ...
            <div class="flex gap-2 flex-wrap pl-2">
              <a v-for="link in project.links" target="_blank" :href="link.url"
                ...
                <div class="flex items-center gap-1">
                  ...
                  <div class="inline relative m-0 p-0 h-5 w-5">
                    <img class=" absolute top-0 left-0 h-5 w-5 group-hover:hidden" :src="getImageUrl(link.icon)" />
          ...
          <div class="projectFeatures py-2 hover:font-bold hover:text-teal-500">
            <button class="float-right" type="button" @click="showProjectModal(project.id)">See All Features
              Here!</button>
          </div>
        </div>
